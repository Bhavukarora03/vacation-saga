...
var value1=last_part1;
  ...
  var urloc4 = window.location.href;
 var partss = urloc4.split("?poid=");
      var last_parts = partss[partss.length-1];
	//var values=last_parts;
	var aa1=last_parts.split('#');
	  var url = 'https://admin.vacationsaga.com/api/properties_image?pid='+value+'&poid='+aa1;
	 // var url ='https://admin.vacationsaga.com/api/properties_image?pid=3156727449531103&poid=112030,830299,624809,546105,834042,days=null';
  ...
  function single_property()
  {
    ...
    response.then((data)=>{
      if(data.length){
        let info=[];
        data.map((item)=>{
        info.push({pdid:item.pdid,propertytitle:item.propertytitle,pstate:item.pstate,pcountry:item.pcountry,ownername:item.name,ownerid:item.ownerid,propertydetal:item.pdiscription,adve_date:item.adve_date,latitude:item.latitude,longitude:item.longitude,ownerdiscription:item.ownerdiscription,speakl:item.speakl,ratings:item.ratings,display_title:item.display_title,agent_ids:item.agent_ids,photo:item.photo,accountmangaer:item.accountmangaer,cancellation_policy:item.cancellation_policy});
            ...
            <span className="text-neutral-900 dark:text-neutral-200 font-medium">
              {(() => {
                if (item.accountmangaer!=undefined) {
                  return (
            <div id="coordinator"><span>Coordinator - {item.accountmangaer} </span></div>  
             )
  }})()}
            </span>
