...
const AccountPage: FC<AccountPageProps> = ({ className = "" }) => {
  ...
  function ownerprofile() {
    var url =
      "https://admin.vacationsaga.com/api/owner_info_api/" +
      cookies.get("ownerid");
    ...
    response
      .then((data) => {
        if (data.length) {
          ...
          data.map(
            (item) => (setdefaultstate(item.state), get_ownerstate(item.state))
          );
          data.map(
            (item) => (setdefaultcity(item.city), get_ownercity(item.city))
          );
        }
      })
      .catch((e) => {
  ...
  function handleClick(event) {
    ...
    let url = fetch("https://admin.vacationsaga.com/api/owner_infoupdate", {
            ...
            >
              ...
              <div className="flex flex-col md:flex-row">
                <div className="flex-shrink-0 flex items-start">
                  <div className="relative rounded-full overflow-hidden flex">
                    <Avatar
                      sizeClass="w-32 h-32"
                      imgUrl={
                        "https://admin.vacationsaga.com/public/images/owner/" +
                        item.photo
                      }
                    />
                ...
                <div className="flex-grow mt-10 md:mt-0 md:pl-16 max-w-3xl space-y-6">
                  ...
                  <div>
                    <Label>Email</Label>
                    <Input className="mt-1.5" readOnly value={item.email} />
                  </div>
